/* C program that accepts students scores (with validation, i.e. between 0 - 100 inclusive) in an exam, calculates and displays the mean score and grade. */
    ...
    for(n = 0; n < numOfStudents; n++)
    {
        ...
        for(a = 0; a < numOfSubjects; a++)
        {
            ...
            while(1)
            {
                ...
                printf("\nPlease enter %s's score in %s (0 - 100)->",studentNames[n],subjectNames[a]);
                if(scanf("%hu",&score) == 0 && score <= 100);
                {
